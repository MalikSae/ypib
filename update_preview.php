<?php
$file = 'resources/views/landing/preview.blade.php';
$lines = file($file);
$headAndHero = implode("", array_slice($lines, 0, 219));

$newSections = <<<'HTML'
    <!-- SECTION 2: STRIP STATISTIK -->
    <section class="py-12 bg-primary-950 relative z-20 shadow-2xl shadow-primary-900/50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-16 text-center">
                <div class="inline-flex items-center gap-3 px-6 py-3 bg-white border border-neutral-200 rounded-2xl shadow-sm text-sm font-bold text-neutral-700">
                    ...
                    Pendaftaran: {{ $period ? \Carbon\Carbon::parse($period->open_date)->translatedFormat('d M Y') . ' — ' . \Carbon\Carbon::parse($period->close_date)->translatedFormat('d M Y') : 'Dibuka Segera' }}
                </div>
            </div>
        </div>
    </section>
    ...
    <!-- SECTION 8: FINAL CTA -->
    <section class="py-24 bg-gradient-to-br from-primary-800 to-primary-950 relative overflow-hidden">
        <!-- Decor -->
        <div class="absolute inset-0 z-0">
            <div class="absolute -top-24 -left-24 w-[500px] h-[500px] bg-primary-600 rounded-full mix-blend-overlay filter blur-[100px] opacity-50"></div>
            <div class="absolute bottom-0 right-0 w-[400px] h-[400px] bg-secondary-500 rounded-full mix-blend-overlay filter blur-[120px] opacity-30"></div>
            <!-- Grid Pattern -->
        ...
        <div class="relative z-10 max-w-4xl mx-auto px-4 text-center">
            <div class="inline-block px-4 py-1.5 mb-6 rounded-full bg-white/10 backdrop-blur-md text-white border border-white/20 text-xs font-bold tracking-widest uppercase">
                PMB UNIVERSITAS YPIB {{ $period->year ?? '2026/2027' }}
            </div>
            ...
            <p class="text-xl text-primary-100 font-medium mb-12 max-w-2xl mx-auto">
                Daftarkan dirimu sekarang sebelum kuota penuh. Pendaftaran ditutup pada <strong class="text-white">{{ $period ? \Carbon\Carbon::parse($period->close_date)->translatedFormat('d F Y') : 'N/A' }}</strong>.
            </p>
            ...
            <div class="flex flex-col sm:flex-row justify-center items-center gap-4 mb-20">
                ...
                @if($period && $period->admin_whatsapp)
...
file_put_contents($file, $headAndHero . "\n" . $newSections);
echo "Berhasil update preview.blade.php";
