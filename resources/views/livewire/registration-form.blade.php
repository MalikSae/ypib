<form wire:submit.prevent="submit" class="max-w-3xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex justify-between relative mb-2">
            ...
            <div class="absolute top-4 left-0 w-full h-0.5 bg-gray-200" style="transform: translateY(-50%); z-index: 0;"></div>
            <!-- Active Connecting Line -->
            <div class="absolute top-4 left-0 h-0.5 transition-all" style="width: {{ ($step - 1) * 25 }}%; background-color:#082e8f; transform: translateY(-50%); z-index: 0;"></div>
            
            @foreach(['Data Diri', 'Prodi & Jalur', 'Asal Sekolah', 'Data Orang Tua', 'Konfirmasi'] as $i => $label)
                @php $num = $i + 1; @endphp
                <div class="flex flex-col items-center flex-1 relative" style="z-index: 10;">
                    <div class="w-8 h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center text-xs md:text-sm font-bold border-2 transition-all
                    ...
                    <div class="hidden md:block text-xs mt-2 font-medium text-center {{ $step >= $num ? '' : 'text-gray-400' }}" style="white-space: nowrap; {{ $step >= $num ? 'color:#082e8f;' : '' }}">{{ $label }}</div>
                </div>
            @endforeach
        </div>
        <!-- Mobile Step Indicator Title -->
        <div class="md:hidden mt-4 text-center">
            @php $labels = ['Data Diri', 'Program Studi & Jalur', 'Asal Sekolah', 'Data Orang Tua / Wali', 'Konfirmasi']; @endphp
            <div class="inline-block px-3 py-1 rounded-full bg-blue-50 border border-blue-100">
                <span class="text-xs font-bold" style="color:#082e8f;">Langkah {{ $step }} dari 5:</span>
                <span class="text-xs font-medium text-gray-700 ml-1">{{ $labels[$step-1] }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <div class="space-y-5">
            <div>
                ...
                <input wire:model="full_name" type="text" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="Sesuai KTP/Ijazah">
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">NISN (10 digit) <span class="text-red-500">*</span></label>
                    <input wire:model="nisn" type="text" maxlength="10" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none" placeholder="00xxxxxxxx">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select wire:model="religion" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-primary-500 focus:ring-primary-500">
            ...
            <div>
                ...
                <textarea wire:model="address" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="Jl. Contoh No. 1, Desa, Kecamatan, Kabupaten"></textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Nomor HP / WhatsApp <span class="text-red-500">*</span></label>
                <input wire:model="phone" type="tel" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="08xxxxxxxxxx">
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <div class="mb-6">
            ...
            <select wire:model.live="first_choice_program_id" class="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-primary-500 focus:ring-primary-500 bg-gray-50">
                <option value="">-- Pilih Program Studi --</option>
                @foreach($programs as $program)
                    <option value="{{ $program->id }}">{{ $program->name }}</option>
                @endforeach
            </select>
            @error('first_choice_program_id') <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span> @enderror
        </div>
        ...
        <div class="mb-8 rounded-2xl overflow-hidden border border-gray-200 shadow-sm bg-white text-left text-gray-800">
            <div class="px-5 py-3 border-b border-gray-100 flex items-center gap-3" style="background-color: #FAFAFA;">
                ...
                <span class="text-xs font-bold tracking-widest text-gray-500 uppercase">Profil Program Studi</span>
            </div>
        ...
        <label class="block text-sm font-medium text-gray-700 mb-3">Pilih Jalur Pendaftaran <span class="text-red-500">*</span></label>
        <style>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <div class="space-y-5">
            <div>
                ...
                <input wire:model="school_name" type="text" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="SMA/SMK/MA Negeri/Swasta ...">
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Jurusan <span class="text-red-500">*</span></label>
                    <input wire:model="school_major" type="text" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="IPA, IPS, Teknik Komputer ...">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">No. Ijazah / SKL <span class="text-red-500">*</span></label>
                    <input wire:model="certificate_number" type="text" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="DN-XX/XX/XXXXXXX">
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tahun Lulus <span class="text-red-500">*</span></label>
                    <input wire:model="graduation_year" type="number" min="2000" max="2030" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="2024">
                </div>
                <div>
                    ...
                    <input wire:model="school_grade" type="number" step="0.01" min="0" max="100" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="85.50">
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input wire:model="father_name" type="text" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="Nama lengkap Ayah">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Ibu <span class="text-red-500">*</span></label>
                    <input wire:model="mother_name" type="text" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="Nama lengkap Ibu">
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Pekerjaan Ayah <span class="text-red-500">*</span></label>
                    <input wire:model="father_occupation" type="text" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="Wiraswasta, PNS, Petani ...">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Pekerjaan Ibu <span class="text-red-500">*</span></label>
                    <input wire:model="mother_occupation" type="text" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none border-primary-600 text-primary-600" placeholder="Ibu Rumah Tangga, PNS ...">
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-gray-50 rounded-xl p-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-2 text-sm">
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Nama Lengkap</span><span class="font-medium">{{ $full_name }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">NISN</span><span class="font-medium">{{ $nisn }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">NIK</span><span class="font-medium">{{ $nik }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Tempat, Tgl Lahir</span><span class="font-medium">{{ $birth_place }}, {{ $birth_date ? \Carbon\Carbon::parse($birth_date)->isoFormat('D MMMM Y') : '-' }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Jenis Kelamin</span><span class="font-medium">{{ $gender === 'male' ? 'Laki-laki' : ($gender === 'female' ? 'Perempuan' : '-') }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Agama</span><span class="font-medium">{{ $religion }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Nomor HP</span><span class="font-medium">{{ $phone }}</span></div>
                    <div class="flex flex-col md:contents"><span class="text-gray-500 text-xs md:text-sm">Alamat</span><span class="font-medium">{{ Str::limit($address, 60) }}</span></div>
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-xl p-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-2 text-sm">
                    ...
                    @endphp
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Program Studi</span><span class="font-medium">{{ $firstProg?->name ?? '-' }}</span></div>
                    <div class="flex flex-col md:contents"><span class="text-gray-500 text-xs md:text-sm">Jalur Pendaftaran</span><span class="font-medium">{{ $pathLabels[$admission_path] ?? '-' }}</span></div>
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-xl p-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-2 text-sm">
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Nama Sekolah</span><span class="font-medium">{{ $school_name }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Jurusan</span><span class="font-medium">{{ $school_major }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">No. Ijazah / SKL</span><span class="font-medium">{{ $certificate_number }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Tahun Lulus</span><span class="font-medium">{{ $graduation_year }}</span></div>
                    <div class="flex flex-col md:contents"><span class="text-gray-500 text-xs md:text-sm">Nilai Rata-rata</span><span class="font-medium">{{ $school_grade ?: '-' }}</span></div>
                </div>
            </div>

            {{-- Data Orang Tua --}}
            <div class="bg-gray-50 rounded-xl p-5">
                <h3 class="font-semibold text-gray-800 mb-3 text-sm uppercase tracking-wide">Data Orang Tua / Wali</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-2 text-sm">
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Nama Ayah</span><span class="font-medium">{{ $father_name }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Pekerjaan Ayah</span><span class="font-medium">{{ $father_occupation }}</span></div>
                    <div class="flex flex-col mb-3 md:mb-0 md:contents"><span class="text-gray-500 text-xs md:text-sm">Nama Ibu</span><span class="font-medium">{{ $mother_name }}</span></div>
                    <div class="flex flex-col md:contents"><span class="text-gray-500 text-xs md:text-sm">Pekerjaan Ibu</span><span class="font-medium">{{ $mother_occupation }}</span></div>
                </div>
            </div>
