<x-guest-layout>
@section('auth-title', 'Masuk — PMB YPIB Majalengka')
<div class="auth-card">
    {{-- Logo --}}
    ...
    <h1 style="font-size:22px;font-weight:700;margin:0 0 4px 0;" class="text-neutral-900">Masuk ke Akun</h1>
    <p style="font-size:14px;margin:0 0 28px 0;" class="text-neutral-500">Selamat datang kembali di PMB YPIB Majalengka</p>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="auth-group">
            ...
            <input id="email" type="email" name="email" class="auth-input {{ $errors->has('email') ? 'auth-input-error' : '' }}"
        ...
        <div class="auth-group">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <label for="password" class="auth-label" style="margin:0;">Kata Sandi</label>
                @if(Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="auth-link" style="font-size:13px;">Lupa sandi?</a>
                @endif
            </div>
            <div style="position:relative;">
                <input id="password" type="password" name="password"
                       class="auth-input {{ $errors->has('password') ? 'auth-input-error' : '' }}"
                       required autocomplete="current-password" placeholder="Masukkan kata sandi" style="padding-right:40px;">
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:24px;">
            <input id="remember_me" type="checkbox" name="remember"
                   style="width:16px;height:16px;accent-cursor:pointer;" class="text-primary-600">
            <label for="remember_me" style="font-size:14px;cursor:pointer;" class="text-neutral-600">Ingat saya</label>
        </div>
    ...
    <p style="text-align:center;font-size:14px;margin:0;" class="text-neutral-500">
        Belum punya akun?
        <a href="{{ route('register') }}" class="auth-link" style="margin-left:4px;">Daftar sekarang</a>
    </p>
</div>
</x-guest-layout>
