...
<style>
    ...
    @media(max-width:767px){ .ref-stat-grid{ grid-template-columns:1fr 1fr; } }
...
<div style="max-width:960px;margin:0 auto;">
    ...
    @php
    $statCards = [
        ['label'=>'Total Klik', 'value'=>$stats['total_clicks'], 'svg'=>'<path stroke-linecap="round" stroke-linejoin="round" d="M15.042 21.672 13.684 16.6m0 0-2.51 2.225.569-9.47 5.227 7.917-3.286-.672Zm-7.518-.267A8.25 8.25 0 1 1 20.25 10.5M8.288 14.212A5.25 5.25 0 1 1 17.25 10.5"/>', 'color'=>'#0064E0','bg'=>'#EFF4FF'],
        ...
        ['label'=>'Total Reward', 'value'=>'Rp '.number_format($stats['total_rewards'],0,',','.'), 'svg'=>'<path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/>', 'color'=>'#0064E0','bg'=>'#EFF4FF'],
        ['label'=>'Reward Dicairkan', 'value'=>'Rp '.number_format($stats['disbursed_rewards'],0,',','.'), 'svg'=>'<path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 0 0-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 0 1-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 0 0 3 15h-.75M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm3 0h.008v.008H18V10.5Zm-12 0h.008v.008H6V10.5Z"/>', 'color'=>'#0064E0','bg'=>'#EFF4FF'],
    ];
    @endphp
    ...
    <div class="pub-card" style="margin-bottom:16px;">
        ...
        <div x-data="{ copied: false }">
            ...
            <div style="background:#F1F4F7;border:1px solid #DEE3E9;border-radius:10px;padding:12px 14px;display:flex;align-items:flex-start;gap:10px;">
                <svg style="width:16px;height:16px;color:#0064E0;flex-shrink:0;margin-top:1px;" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <div style="background:#FFFFFF;border:1px solid #DEE3E9;border-radius:16px;overflow:hidden;">
        ...
        @if($registrations->isEmpty())
            <div style="padding:48px 24px;text-align:center;">
                <div style="width:48px;height:48px;border-radius:14px;background:#EFF4FF;display:flex;align-items:center;justify-content:center;margin:0 auto 12px;">
                ...
                <table class="table-nowrap" style="width:100%;border-collapse:collapse;">
                    ...
                    <tbody>
                        ...
                        $rw = [
                            ...
                            'disbursed' => ['Dicairkan', '#EFF4FF', '#0064E0'],
                        ];
                        @endphp
                        <tr style="border-bottom:1px solid #DEE3E9;transition:background 0.12s;"
                            ...
                            <td style="padding:14px 20px;font-size:14px;font-weight:700;color:#0A1317;">
                                @if($reg->reward)
                                    Rp {{ number_format($reg->reward->amount, 0, ',', '.') }}
                                @else
                            ...
                            <td style="padding:14px 20px;">
                                @if($reg->reward)
                                    @php $rs = $rw[$reg->reward->status] ?? ['—', '#F1F4F7', '#5D6C7B']; @endphp
